...
export async function POST(request: NextRequest) {
    try {
        ...
        try {
            ...
            console.error('[MT5Worker/Trades] Invalid JSON payload:', error);
            return NextResponse.json(
        ...
        if (!validationResult.success) {
            console.error('[MT5Worker/Trades] Validation error:', validationResult.error);
            return NextResponse.json(
                {
                    success: false,
        ...
        await recordMt5WorkerTradesResult(
            data.terminalId,
            request,
            result,
            data.syncCursor
        ...
        console.error('[MT5Worker/Trades] Error:', error);
        return NextResponse.json(
