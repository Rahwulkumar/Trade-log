...
export default function StrategiesPage() {
  ...
  const DEMO_STRATEGY: PlaybookWithStats = {
    id: "demo-1",
    user_id: "demo-user",
    name: "Trend Following Breakout",
    description: "Captures high-momentum moves after consolidation periods. Best used in trending markets with clear volume confirmation.",
    created_at: new Date().toISOString(),
    is_active: true,
    rules: ["Price must be above 200 EMA", "Wait for consolidation > 5 candles", "Enter on breakout with volume > 1.5x avg"],
    stats: {
      totalTrades: 42,
      winRate: 64.5,
      avgRMultiple: 2.4,
  ...
  const [playbooks, setPlaybooks] = useState<PlaybookWithStats[]>([DEMO_STRATEGY]);
  ...
  async function loadPlaybooks() {
    ...
    try {
      ...
      setPlaybooks(playbooksWithStats.length > 0 ? playbooksWithStats : [DEMO_STRATEGY]);
    } catch (e) {
  ...
  async function handleCreateDemo() {
    if(!confirm("Create a sample 'ICT Silver Bullet' strategy?")) return;
    setLoading(true);
    try {
        await createPlaybook({
            name: "ICT Silver Bullet (Demo)",
            description: "A time-based liquidity run strategy targeting fair value gaps (FVG) during the NY Session AM killzone.",
            rules: [
                "Time: 10:00 AM - 11:00 AM NY Time",
                "Setup: Wait for liquidity sweep (PDH/PDL)",
                "Confirmation: Displacement + FVG creation",
                "Entry: Limit order at FVG retest",
                "Stop: Below swing low/high of displacement leg"
            ],
            is_active: true
        });
        await loadPlaybooks();
  ...
  function startCreating() {
    setMessages([{ id: "1", role: "assistant", content: "What trading strategy would you like to create?" }]);
    setGenerated(null);
    setInput("");
    setIsCreating(true);
  }
  ...
  async function send() {
    ...
    setMessages(p => [...p, { id: Date.now().toString(), role: "user", content: input }]);
    setInput("");
    setThinking(true);
    ...
    try {
      const res = await fetch("/api/ai", {
        ...
        body: JSON.stringify({ action: "generate-strategy", prompt: input, messages }),
      });
      ...
      if (data.success && data.strategy) {
        ...
        setMessages(p => [...p, { id: Date.now().toString(), role: "assistant", content: data.error || "Error" }]);
      }
    } catch {
      setMessages(p => [...p, { id: Date.now().toString(), role: "assistant", content: "Failed" }]);
    } finally {
      setThinking(false);
    }
  }
        ...
        <div className="p-3">
          ...
          >
           ...
           <button 
            ...
            className="w-full mt-2 px-4 py-2 text-xs font-medium text-muted-foreground hover:text-white border border-dashed border-white/10 hover:border-white/20 rounded-lg transition-colors flex items-center justify-center gap-2"
          >
        ...
        <div className="flex-1 overflow-y-auto px-2 space-y-0.5">
          {loading ? (
            ...
            playbooks.map((p) => (
              ...
              >
                <div className={cn("w-1.5 h-1.5 rounded-full flex-shrink-0", p.is_active ? "bg-green-500/50" : "bg-white/10")} />
                ...
                <div className="max-w-3xl mx-auto p-4">
                  <div className="p-4 bg-green-500/5 border border-green-500/20 rounded-xl">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-xs text-green-400 uppercase tracking-wide mb-1 font-semibold">Strategy Ready</p>
                        <p className="font-medium text-lg text-white">{generated.name}</p>
                        <p className="text-xs text-muted-foreground mt-1">{generated.rules.length} rules defined</p>
                      </div>
          ...
          <div className="p-8">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-muted-foreground">Manage and track your trading playbooks</p>
              </div>
            ...
            {loading ? (
              ...
              <div className="card-void p-12 text-center max-w-lg mx-auto mt-10">
                <div className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center mx-auto mb-6">
                  <BookOpen className="h-8 w-8 text-muted-foreground/50" />
                </div>
                <h3 className="text-xl font-semibold mb-2">No strategies yet</h3>
                <p className="text-muted-foreground mb-6">Use our AI assistant to help you create your first structured trading strategy.</p>
                <button className="btn-glow" onClick={startCreating}>
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4">
                {playbooks.map((playbook) => (
                  <div key={playbook.id} className={cn("card-void p-6 relative group hover:border-white/10 transition-colors", !playbook.is_active && "opacity-60")}>
                    <div className="flex items-start justify-between mb-5">
                      <div className="flex items-center gap-3">
                        <div className={cn(
                          "p-2.5 rounded-xl transition-colors", 
                          (playbook.stats?.totalPnl || 0) >= 0 ? "bg-green-500/10 group-hover:bg-green-500/20" : "bg-red-500/10 group-hover:bg-red-500/20"
                        )}>
                          <BookOpen className={cn("h-4.5 w-4.5", (playbook.stats?.totalPnl || 0) >= 0 ? "text-green-500" : "text-red-500")} />
                        </div>
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1.5">
                            ...
                            {!playbook.is_active && <span className="badge-void text-[10px] py-0.5 h-auto">Inactive</span>}
                          </div>
                        </div>
                      </div>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="bg-[#0a0a0a] border-white/10 min-w-[160px]">
                          ...
                          <DropdownMenuItem className="text-red-500 focus:text-red-400 focus:bg-red-500/10" onClick={() => remove(playbook.id)}>
                    ...
                    <div className="mb-5">
                      <div className="flex justify-between text-xs mb-2">
                        ...
                        <span className="font-medium text-white">{(playbook.stats?.winRate || 0).toFixed(1)}%</span>
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center mb-5 p-3 rounded-lg bg-white/[0.02] border border-white/[0.02]">
                      <div>
                        <div className="text-base font-semibold text-white">{playbook.stats?.totalTrades || 0}</div>
                        <div className="text-[10px] uppercase tracking-wider text-muted-foreground mt-0.5">Trades</div>
                      </div>
                      <div>
                        <div className="text-base font-semibold text-white">{(playbook.stats?.avgRMultiple || 0).toFixed(1)}R</div>
                        <div className="text-[10px] uppercase tracking-wider text-muted-foreground mt-0.5">Avg R</div>
                      </div>
                      <div>
                        <div className={cn("text-base font-semibold", (playbook.stats?.totalPnl || 0) >= 0 ? "profit" : "loss")}>
                          ${Math.abs(playbook.stats?.totalPnl || 0).toLocaleString()}
                        </div>
                        <div className="text-[10px] uppercase tracking-wider text-muted-foreground mt-0.5">P&L</div>
                      </div>
                    </div>
                    ...
                    {playbook.rules && playbook.rules.length > 0 && (
                      <div className="flex items-center gap-2 mb-0">
                        <span className="text-xs text-muted-foreground">
                            {playbook.rules.length} {playbook.rules.length === 1 ? 'rule' : 'rules'} defined
                        </span>
                      </div>
                    )}
